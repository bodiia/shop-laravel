        ...
        <div class="container">
            ...
            <section>
                            ...
                            <tr>
                                ...
                                <td class="py-4 px-4 md:px-6 bg-card">
                                    <div class="flex items-stretch h-[56px] gap-2">
                                        <form action="{{ route('cart.product_quantity', $cartItem) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <input name="quantity" type="number" class="h-full px-2 lg:px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition" min="1" max="999" value="{{ $cartItem->quantity }}" placeholder="К-во">
                                        </form>
                                    </div>
                                </td>
                                <td class="py-4 px-4 md:px-6 bg-card">
                                    <div class="font-medium whitespace-nowrap">{{ $cartItem->amount_price }}</div>
                                </td>
                                <td class="py-4 px-4 md:px-6 rounded-r-2xl bg-card">
                                    <form action="{{ route('cart.product_destroy', $cartItem) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button class="w-12 !h-12 !px-0 btn btn-pink" title="Удалить из корзины">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-8">
                    <div class="text-[32px] font-black">Итого: {{ $cart->total_price }}</div>
