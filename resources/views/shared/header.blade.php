    ...
    <div class="container">
        <div class="header-inner flex items-center justify-between lg:justify-start">
            ...
            <div class="header-menu grow hidden lg:flex items-center ml-8 mr-8 gap-8">
                ...
                <nav class="flex gap-x-4">
                    @foreach($menu as $item)
                        <a href="{{ $item->getUrl() }}" class="text-white hover:text-pink @if($item->isActive()) font-bold @endif">{{ $item->getLabel() }}</a>
                    @endforeach
                </nav>
            </div><!-- /.header-menu -->
            <div class="header-actions flex items-center gap-3 md:gap-5">
                ...
                @guest
                    <a href="{{ route('signin.index') }}" class="profile hidden xs:flex items-center">
                ...
                @auth
                    ...
                    <button @click="dropdownProfile = ! dropdownProfile" class="flex items-center text-white hover:text-pink transition">
                        ...
                        <img src="{{ auth()->user()->profile_image }}" class="shrink-0 w-7 md:w-9 h-7 md:h-9 rounded-full" alt="{{ auth()->user()->name }}">
                    ...
                    >
                        ...
                        <div class="flex items-center mt-3">
                            <img src="{{ auth()->user()->profile_image }}" class="w-11 h-11 rounded-full" alt="Данил Шуцкий">
                ...
                <a href="{{ route('cart.index') }}" class="flex items-center gap-3 text-pink hover:text-white">
                    ...
                    <div class="hidden sm:flex flex-col gap-2">
                        <span class="text-white text-xxs 2xl:text-xs font-bold !leading-none">{{ get_current_cart(auth()->user())?->total_price ?? '0 ₽' }}</span>
                    </div>
                </a>
            </div><!-- /.header-actions -->
        </div><!-- /.header-inner -->
